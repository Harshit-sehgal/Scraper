...
from app.job_store import load_state, reset_job_store_for_tests, _get_connection
...
def run_drill():
    ...
    with conn:
        conn.execute(
            ...
            (JobStatus.RUNNING.value, '[]', crash_job_id)
        )
    ...
    assert "restart" in interrupted_job.error.lower() or "recovery" in interrupted_job.error.lower() or "recovered" in interrupted_job.error.lower(), f"Unexpected error message: {interrupted_job.error}"
