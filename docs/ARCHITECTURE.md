...
| Extraction | `backend/app/extraction_orchestrator.py`, `backend/app/selector_engine.py`, `backend/app/network_extractor.py`, `backend/app/container_discovery.py`, `backend/app/rendered_visible_text_extractor.py`, `backend/app/schema_*.py` | Schema, selector, network payload, text fallback extraction | Core | Unit/API/browser tests | Real-world accuracy not proven | Keep, add benchmarks |
| Cleaning/enrichment | `backend/app/cleaning_engine.py`, validators | Normalize and validate extracted records | Stable supporting | Safe suite | Quality varies by site/schema | Keep |
...
| Telemetry/diagnostics | `backend/app/scrape_telemetry.py`, `backend/app/metrics_collector.py`, router endpoints | Observability, diagnostics, Prometheus metrics | Stable supporting | Safe suite, route matrix, local Prometheus target check | Target-network exposure still needs validation | Keep, verify target deployment |
