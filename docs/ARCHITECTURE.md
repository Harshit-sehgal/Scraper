...
| Scraper/browser | `backend/app/scraper.py`, `backend/app/browser.py`, `backend/app/browser_pool.py` | HTTP/browser fetch, Playwright orchestration, page loading | Core | Browser suite: `1858 passed, 55 skipped`; container Chromium smoke | Broad real-world extraction not proven | Keep, add benchmark corpus |
