...
| DEPS-MIG | Migrate to `pyproject.toml`-only dependency workflow | ✅ | Removed all `backend/requirements*.in` / `.lock.txt` / `.txt` files; `pyproject.toml` is now the single source of truth (PEP 621). `Dockerfile`, all 9 GitHub workflows, `Makefile`, `scripts/start.sh`, `scripts/doctor.py`, `scripts/validate_dependency_bounds.py` updated. Docs (`README.md`, `SETUP.md`, `PRODUCTION.md`, `ROADMAP.md`) aligned. `pip install .` for prod, `pip install -e ".[dev]"` for dev. | this session |
