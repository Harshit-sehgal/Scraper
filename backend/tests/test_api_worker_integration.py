...
# ─────────────────────────────────────────────────────────────────────
# Phase 2 — End-to-end API → Worker handler execution test
# ─────────────────────────────────────────────────────────────────────


class TestApiWorkerHandlerExecution:
    """End-to-end test: API creates job, worker handler executes with mocked scrape."""

    def test_api_enqueues_and_worker_handler_executes_job(
        self, tmp_path, monkeypatch
    ):
        """
        Full end-to-end test:
        1. Enable DATAFORGE_WORKER_QUEUE=true
        2. Mock scrape_url_with_recovery to return sample records
        3. Create job through API
        4. Dequeue task from WorkerQueue
        5. Execute handler that calls run_job with mocked scrape
        6. Assert job becomes COMPLETED with records
        """
        # ── Setup: mock the scraping layer ────────────────────────────────
        ...
        # Patch both the module-level reference and the already-imported
        # reference in job_runner.py (which was bound at import time).
        monkeypatch.setattr(
        ...
        from app.worker_queue import reset_worker_queue, get_worker_queue, Priority
        reset_worker_queue()
        queue = get_worker_queue(db_path=tmp_path / "test_worker_e2e.db")

        # ── Setup: register the real handler path ────────────────────────
        from app.services.job_runner import run_job

        async def scrape_job_handler(task):
            from app.storage_interface import get_job_repository
            from app.config import settings as cfg

            job_id = task.payload.get("job_id")
            if not job_id:
                return {"error": "no_job_id"}

            repo = get_job_repository()
            loaded_jobs, loaded_recycle, _ = repo.load_all()
            job = loaded_jobs.get(job_id)
            if not job:
                return {"error": "job_not_found"}

            await run_job(
                job_id=job_id,
                jobs_store=loaded_jobs,
                persist_state_fn=lambda: repo.save_all(loaded_jobs, loaded_recycle),
                max_discovery_urls=cfg.MAX_DISCOVERY_URLS,
                max_job_runtime_seconds=cfg.MAX_JOB_RUNTIME_SECONDS,
                per_url_scrape_timeout_seconds=cfg.PER_URL_TIMEOUT_SECONDS,
                ai_structuring_timeout_seconds=cfg.AI_STRUCTURING_TIMEOUT_SECONDS,
                insight_timeout_seconds=cfg.INSIGHT_TIMEOUT_SECONDS,
                persist_state_single_fn=lambda: repo.save_single(loaded_jobs[job_id]),
                persist_state_single_critical_fn=lambda: repo.save_single(loaded_jobs[job_id]),
            )
            return {
                "job_id": job_id,
                "status": loaded_jobs[job_id].status.value,
                "total_records": loaded_jobs[job_id].total_records,
            }
        ...
        # The job may be COMPLETED or DEGRADED depending on how scraped
        # records are processed through filters/AI structuring.
        # The key assertion is that processing completed without error.
        assert result["status"] in ("completed", "degraded"), (
            f"Expected terminal status, got {result['status']}"
        )

        # ── Step 5: Verify repository contains updated job ───────────────
        from app.storage_interface import get_job_repository
        repo = get_job_repository()
        loaded_jobs, loaded_recycle, _ = repo.load_all()
