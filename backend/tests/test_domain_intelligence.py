...
def test_moving_average_hydration(registry):
    ...
    # First update: 2000ms
    registry.update_from_telemetry({"url": url, "js_render_delay_ms": 2000.0, "anti_bot_score": 0.0, "fallback_usage": "none"})

    intel = registry.get_intelligence(url)
    assert intel.hydration_delay_ms == 600.0  # 0.3 * 2000

    # Second update: 1000ms
    registry.update_from_telemetry({"url": url, "js_render_delay_ms": 1000.0, "anti_bot_score": 0.0, "fallback_usage": "none"})
