...
class TestExtractFromNetwork:
    ...
    def test_returns_empty_list_for_empty_data(self, sample_schema) -> None:
        result = extract_from_network({}, sample_schema)
        assert result == []

    def test_extracts_from_jsonld_products(self, sample_schema) -> None:
    ...
    def test_extracts_from_network_payloads(self, sample_schema) -> None:
    ...
    def test_scored_and_sorted(self, sample_schema) -> None:
        data = {
            "jsonld": [
...
class TestExtractFromJsonld:
    def test_extracts_from_graph(self, sample_schema) -> None:
        jsonld = [{"@graph": [{"@type": "Product", "name": "G1"}, {"@type": "Product", "name": "G2"}]}]
        result = _extract_from_jsonld(jsonld, sample_schema)
        assert len(result) == 2

    def test_extracts_from_item_list(self, sample_schema) -> None:
        jsonld = [{"itemListElement": [{"@type": "Product", "name": "I1"}, {"@type": "Product", "name": "I2"}]}]
        result = _extract_from_jsonld(jsonld, sample_schema)
        assert len(result) == 2

    def test_extracts_from_has_part(self, sample_schema) -> None:
        jsonld = [{"hasPart": [{"@type": "Article", "name": "Part1"}]}]
        result = _extract_from_jsonld(jsonld, sample_schema)
        assert len(result) >= 1

    def test_returns_empty_for_empty_list(self, sample_schema) -> None:
        assert _extract_from_jsonld([], sample_schema) == []

    def test_returns_empty_for_non_dict_items(self, sample_schema) -> None:
        assert _extract_from_jsonld([{"@graph": ["not_a_dict"]}], sample_schema) == []

    def test_extracts_flat_json_as_single_record(self, sample_schema) -> None:
...
class TestMapJsonldItem:
    def test_product_type_handler(self, sample_schema) -> None:
    ...
    def test_offer_type_handler(self, sample_schema) -> None:
        item = {"@type": "Offer", "price": "49.99", "priceCurrency": "USD", "name": "Special Offer"}
        result = _map_jsonld_item(item, sample_schema)
        assert result is not None

    def test_flight_type_handler(self, sample_schema) -> None:
        item = {
            "@type": "Flight",
            "flightNumber": "AA123",
    ...
    def test_hotel_type_handler(self, sample_schema) -> None:
        item = {"@type": "Hotel", "name": "Grand Hotel", "telephone": "+1-555-0100"}
        result = _map_jsonld_item(item, sample_schema)
        assert result is not None

    def test_job_type_handler(self, sample_schema) -> None:
        item = {"@type": "JobPosting", "title": "Engineer", "hiringOrganization": {"name": "Corp Inc"}}
        result = _map_jsonld_item(item, sample_schema)
        assert result is not None
        assert result.get("title") == "Engineer"

    def test_event_type_handler(self, sample_schema) -> None:
        item = {"@type": "Event", "name": "Concert", "startDate": "2025-07-04"}
        result = _map_jsonld_item(item, sample_schema)
        assert result is not None

    def test_business_type_handler(self, sample_schema) -> None:
        item = {"@type": "LocalBusiness", "name": "Shop", "telephone": "+1-555-0200"}
        result = _map_jsonld_item(item, sample_schema)
        assert result is not None

    def test_person_type_handler(self, sample_schema) -> None:
        item = {"@type": "Person", "name": "John Doe", "email": "john@example.com"}
        result = _map_jsonld_item(item, sample_schema)
        assert result is not None
        assert result.get("name") == "John Doe"

    def test_article_type_handler(self, sample_schema) -> None:
        item = {"@type": "Article", "headline": "Breaking News", "datePublished": "2025-01-01"}
        result = _map_jsonld_item(item, sample_schema)
        assert result is not None

    def test_book_type_handler(self, sample_schema) -> None:
        item = {"@type": "Book", "name": "The Book", "isbn": "978-3-16-148410-0"}
        result = _map_jsonld_item(item, sample_schema)
        assert result is not None

    def test_movie_type_handler(self, sample_schema) -> None:
        item = {"@type": "Movie", "name": "The Movie"}
        result = _map_jsonld_item(item, sample_schema)
        assert result is not None

    def test_app_type_handler(self, sample_schema) -> None:
        item = {"@type": "SoftwareApplication", "name": "MyApp"}
        result = _map_jsonld_item(item, sample_schema)
        assert result is not None

    def test_returns_none_for_empty_item(self, sample_schema) -> None:
        assert _map_jsonld_item({}, sample_schema) is None

    def test_returns_none_for_item_with_no_meaningful_values(self, sample_schema) -> None:
...
class TestExtractFromNestedJson:
    def test_finds_nested_record_arrays(self, sample_schema) -> None:
        data = {
            "data": {
                "products": [
    ...
    def test_returns_empty_for_empty_data(self, sample_schema) -> None:
        assert _extract_from_nested_json({}, sample_schema) == []

    def test_returns_empty_for_non_dict_data(self, sample_schema) -> None:
        # Type-safe: pass empty dict since the signature expects dict
        assert _extract_from_nested_json({}, sample_schema) == []

    def test_respects_max_depth(self, sample_schema) -> None:
        deep = {"l1": {"l2": {"l3": {"l4": {"l5": {"l6": {"name": "Too Deep"}}}}}}}
        result = _extract_from_nested_json(deep, sample_schema)
        assert result == []

    def test_finds_top_level_record_array(self, sample_schema) -> None:
...
class TestExtractFromApolloState:
    def test_extracts_from_apollo_entities(self, sample_schema) -> None:
        data = {
    ...
    def test_extracts_from_root_query_refs(self, sample_schema) -> None:
        data = {
...
class TestExtractRecordsFromPayloads:
    def test_extracts_from_payload_body(self, sample_schema) -> None:
        payloads = [{"body": {"results": [{"name": "Payload Item"}]}}]
        result = _extract_records_from_payloads(payloads, sample_schema)
        assert len(result) > 0

    def test_returns_empty_for_empty_payloads(self, sample_schema) -> None:
        assert _extract_records_from_payloads([], sample_schema) == []

    def test_returns_empty_for_non_dict_body(self, sample_schema) -> None:
        payloads = [{"body": "not a dict"}]
        result = _extract_records_from_payloads(payloads, sample_schema)
        assert result == []

    def test_handles_list_body(self, sample_schema) -> None:
        payloads = [{"body": [{"name": "List Item 1"}, {"name": "List Item 2"}]}]
        result = _extract_records_from_payloads(payloads, sample_schema)
        assert len(result) == 2

    def test_deduplicates(self, sample_schema) -> None:
...
class TestDeduplicateRecords:
    ...
    def test_skips_records_without_significant_values(self) -> None:
        records = [{"x": "y"}, {"x": "y"}]
        result = _deduplicate_records(records)
...
class TestMapJsonKeysToSchema:
    def test_direct_match(self, sample_schema) -> None:
    ...
    def test_alias_match(self, sample_schema) -> None:
        item = {"title": "Alias Match"}
        result = _map_json_keys_to_schema(item, sample_schema)
        assert result.get("name") == "Alias Match"

    def test_returns_empty_for_empty_item(self, sample_schema) -> None:
        assert _map_json_keys_to_schema({}, sample_schema) == {}

    def test_returns_empty_for_non_dict(self, sample_schema) -> None:
        # Type-safe: pass empty dict since the signature expects dict
        assert _map_json_keys_to_schema({}, sample_schema) == {}
...
class TestFlattenJsonKeys:
    ...
    def test_handles_string_lists(self) -> None:
