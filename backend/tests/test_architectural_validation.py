...
class ArchitecturalValidator:
    ...
    def _detect_layer(self, module_name: str) -> str:
        ...
        if any(x in module_name for x in ['selector', 'extraction', 'dom_', 'xpath', 'css_', 'cleaning']):
            return 'Extract'
        elif any(x in module_name for x in ['memory', 'state_', 'cache_', 'checkpoint', 'graph_state', 'persistent', 'world_snapshot']):
            return 'Memory'
        elif any(x in module_name for x in ['llm', 'semantic_', 'job_runner', 'anti_bot', 'behavior', 'extractor', 'content_', 'discovery', 'strategy_', 'domain_evolution']):
            return 'Intelligence'
