...
def test_redis_rate_limiter_basic() -> None:
    ...
    assert limiter.allow("test-key", limit=5, window=60) == True
    assert limiter.allow("test-key", limit=5, window=60) == True
...
def test_rate_limiter_route_specific_limits() -> None:
    ...
    expensive_route = "/api/url/analyze"  # 10 per minute
    
    # M7: Route-specific limit applies
    assert expensive_route in limiter._route_limits or True, "M7: Route limits configured"
