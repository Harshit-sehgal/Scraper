...
class InstabilityState:
    ...
    def set_exclusion(self, key: tuple, value: float):
        sk = tuple(sorted(key))
    ...
    def add_exclusion(self, role_a: str, role_b: str, strength: float):
        key = tuple(sorted([role_a, role_b]))
        target = self._staging if self._staging is not None else self._exclusions
    ...
    def decay_exclusion(self, role_a: str, role_b: str, rate: float = 0.05):
        key = tuple(sorted([role_a, role_b]))
        target = self._staging if self._staging is not None else self._exclusions
        if key in target:
