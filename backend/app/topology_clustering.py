...
def detect_communities(state: "TopologyState") -> None:
    ...
    cohesion = state.get_struct("neighborhood_cohesion")
    ...
    state.set_struct("communities", communities)
...
def shard_topology_regions(state: "TopologyState") -> dict[str, list[str]]:
    ...
    """
    communities = state.get_struct("communities")
    role_to_shard = {}
...
def update_schema_patterns(state: "TopologyState", exclusion_key: tuple, exclusion_val: float) -> None:
    ...
    state.set_struct("schema_patterns", struct)
...
def decay_topological_laws(state: "TopologyState") -> None:
    ...
    struct = state.get_struct("topological_laws")
...
def set_topological_law(state: "TopologyState", pair: tuple, value: float) -> None:
    ...
    laws = state.get_struct("topological_laws")
    laws[tuple(sorted(pair))] = _clamp_signed(value)
    state.set_struct("topological_laws", laws)
    state.record("set_topological_law", {"pair": pair, "value": value})


def add_impossible_neighborhood(state: "TopologyState", item: set[str]) -> None:
    """Add an impossible neighborhood set."""
    struct = state.get_struct("impossible_neighborhoods")
    struct.append(set(item))
    state.set_struct("impossible_neighborhoods", struct)
...
def self_prune_regions(
    ...
    for r in regs:
        ...
        if is_noise and community_required and not has_community:
            state.record("prune_dead_zone", {"region_id": r.region_id, "instability": r.instability})
...
def induce_topological_laws(
    ...
    """
    for pair, success in state.get_cohesion_merge_success().items():
        attempts = state.get_cohesion_merge_attempts().get(pair, 0)
        if attempts >= min_attempts:
            ...
            if rate >= min_success_rate:
                ...
                state.record("induce_law", {"pair": pair, "type": "affinity", "rate": rate})

    for pair, success in state.get_cohesion_split_success().items():
        attempts = state.get_cohesion_split_attempts().get(pair, 0)
        if attempts >= min_attempts:
            rate = success / attempts
            if rate >= min_success_rate:
                current = state.topological_laws.get(pair, 0.0)
                set_topological_law(state, pair, min(current, -0.5 * rate))
                state.record("induce_law", {"pair": pair, "type": "repulsion", "rate": rate})
