...
def create_jobs_router(
    ...
    async def reclean_job(job_id: str, _role: UserRole = Depends(require_role([UserRole.ADMIN, UserRole.OPERATOR]))):
        ...
        # Wrap the entire reclean body so an unexpected failure (e.g. in
        # ``process_results``, ``save_job_results_to_disk`` or ``_save_job``)
        # can't strand the job in ``RUNNING`` forever — which would block
        # future reclean / delete attempts.
        try:
            try:
    ...
    async def clear_recycle_bin(_role: UserRole = Depends(require_role([UserRole.ADMIN]))):
        ...
        with _store_lock:
            snapshot = list(recycle_bin_store.items())
            count = len(snapshot)
