...
class DecayPrediction:
    ...
    failure_velocity: float    # Rate of failure acceleration (positive = worsening)
...
class SelectorDecayPredictor:
    ...
    def predict_decay(self, domain: str) -> DecayPrediction:
        ...
        if len(recent_window) >= 2:
            ...
            newer_avg = sum(s[1] for s in recent_window[len(recent_window)//2:]) / max(1, len(recent_window) - len(recent_window)//2)
        ...
        decay_risk = (
            ...
            min(1.0, failure_rate + velocity) * 0.40 +
            age_risk * 0.30
        )
        ...
        recommendations = self._generate_recommendations(
            domain, risk_level, decay_risk, velocity, confidence_score
        )
    ...
    def get_decay_report(self) -> dict:
        ...
        return {
            ...
            "avg_decay_risk": round(
                sum(p.decay_risk for p in predictions) / max(1, len(predictions)), 3
            ),
