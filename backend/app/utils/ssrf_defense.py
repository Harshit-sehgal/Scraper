...
class SSRFDefense:
    ...
    @classmethod
    def resolve_and_check(cls, hostname: str) -> Optional[str]:
        ...
        try:
            ...
            for family, socktype, proto, canonname, sockaddr in result:
                ...
                if cls.is_blocked_ip(ip):
                    logger.warning(
                        f"SSRF blocked: hostname {hostname} resolved to blocked IP {ip}"
                    )
                    return None
...
class DNSRebindingDefense:
    ...
    def check_dns_rebinding(self, hostname: str) -> bool:
        ...
        try:
                ...
                if current_time - cache_time < self.ttl_cache_seconds:
                    ...
                    try:
                        ...
                        if current_ips != set(cached_ips):
                            logger.warning(
                                f"DNS_REBINDING detected: {hostname} changed from "
                                f"{cached_ips} to {current_ips}"
                            )
                            return True
            ...
            logger.error(f"DNS rebinding check error: {e}")
            return False
