...
class SelfTuningController:
    ...
    def _tune(self, domain: str) -> None:
        ...
        if fetch_times:
            ...
            new_timeout = min(self._max_timeout, max(
                self._min_timeout,
                (avg_fetch / 1000.0) * 2.0 + 5.0
            ))
        ...
        new_delay = min(self._max_delay, max(
            self._min_delay,
        ...
        if quality_scores and len(quality_scores) >= 3:
            ...
            if avg_quality > 0.7:
                params.min_confidence_threshold = min(
                    self._max_confidence,
                    params.min_confidence_threshold + 0.02
                )
            elif avg_quality < 0.3:
                params.min_confidence_threshold = max(
        ...
        logger.debug(
            ...
            domain, params.fetch_timeout_s, params.delay_between_requests_s,
            params.max_retries, params.min_confidence_threshold,
        )
    ...
    def get_tuning_report(self) -> dict:
        ...
        return {
            ...
            "domains": [
                p.to_dict() for p in self._parameters.values()
            ],
            "averages": {
                "avg_timeout_s": round(
                    mean(p.fetch_timeout_s for p in self._parameters.values()), 1
                ) if self._parameters else 0.0,
                "avg_delay_s": round(
                    mean(p.delay_between_requests_s for p in self._parameters.values()), 1
                ) if self._parameters else 0.0,
                "avg_retries": round(
                    mean(p.max_retries for p in self._parameters.values()), 1
                ) if self._parameters else 0.0,
                "avg_confidence_threshold": round(
                    mean(p.min_confidence_threshold for p in self._parameters.values()), 3
                ) if self._parameters else 0.0,
            },
        }
